...
type CropPos = "topLeft"|"topCenter"|"topRight"|"midLeft"|"center"|"midRight"|"botLeft"|"botCenter"|"botRight";
...
// clipAmount: 각 바깥쪽에서 클리핑할 % (클수록 더 가려짐)
function getClipPath(clipAmount: number, hintLevel: number, pos: CropPos): string {
  const scale = hintLevel === 0 ? 1 : hintLevel === 1 ? 0.42 : 0;
  const c = clipAmount * scale;
  const h = c / 2;
  const map: Record<CropPos, [number,number,number,number]> = {
    topLeft:    [0, c, c, 0],
    topCenter:  [0, h, c, h],
    topRight:   [0, 0, c, c],
    midLeft:    [h, c, h, 0],
  ...
  const [t, r, b, l] = map[pos];
  return `inset(${t.toFixed(1)}% ${r.toFixed(1)}% ${b.toFixed(1)}% ${l.toFixed(1)}% round 16px)`;
}
...
const LEVEL_CONFIG: Record<Level, {
  ...
  questionsPerRound:number; clipAmount:number; difficulties:number[];
}> = {
  1: { requiredSuccess:3,        timeLimit:40, label:"레벨 1", stars:"⭐",          questionsPerRound:3, clipAmount:35, difficulties:[1] },
  2: { requiredSuccess:5,        timeLimit:35, label:"레벨 2", stars:"⭐⭐",         questionsPerRound:4, clipAmount:43, difficulties:[1,2] },
  3: { requiredSuccess:7,        timeLimit:30, label:"레벨 3", stars:"⭐⭐⭐",        questionsPerRound:5, clipAmount:50, difficulties:[2,3] },
  4: { requiredSuccess:10,       timeLimit:25, label:"레벨 4", stars:"⭐⭐⭐⭐",       questionsPerRound:5, clipAmount:56, difficulties:[3,4] },
  5: { requiredSuccess:Infinity, timeLimit:20, label:"레벨 5", stars:"⭐⭐⭐⭐⭐",      questionsPerRound:5, clipAmount:62, difficulties:[4,5] },
};

const THEME = {
  bg:"linear-gradient(180deg, #ECFDF5 0%, #D1FAE5 100%)",
  accent:"#10b981", accent2:"#34d399",
  ink:"#064E3B", sub:"rgba(6,78,59,0.55)",
...
export default function PictureGame() {
  ...
  const loadQuestion = useCallback((lvl: Level, shown: string[]) => {
    ...
    const pos = CROP_POSITIONS[Math.floor(Math.random() * CROP_POSITIONS.length)];
    setCurrentItem(q);
  ...
  const clipPath = currentItem ? getClipPath(config.clipAmount, hintLevel, cropPos) : "";
    ...
    <div style={{ height:844, display:"flex", flexDirection:"column", background:THEME.bg, fontFamily:'"Pretendard", -apple-system, system-ui' }}>
      ...
      <div style={{ flex:1, padding:"10px 20px", display:"flex", flexDirection:"column", gap:12 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ width:240, height:240, borderRadius:28, background:"white", boxShadow:"0 12px 32px rgba(16,185,129,0.2)", display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden", position:"relative" }}>
            {!imgLoaded && (
            ...
            {currentItem && (
              <img
                ...
                style={{
                  ...
                  clipPath,
                  transition:"clip-path 0.5s cubic-bezier(.2,.9,.25,1.2)",
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
          {[0,1,2].map(i => (
            <div key={i} style={{ width:10, height:10, borderRadius:99, background:i<=hintLevel?THEME.accent:"rgba(16,185,129,0.2)", transition:"background 0.3s" }} />
          ))}
          <span style={{ fontSize:12, color:THEME.sub, marginLeft:6 }}>
            {hintLevel===0?"힌트를 눌러 더 보기":hintLevel===1?"조금 더 공개됐어요!":"거의 다 보여요!"}
          </span>
        </div>
